...
public class ReservationsController {
	...
	String reserve(@Validated ReservationForm form, BindingResult bindingResult,
		...
		return "redirect:/reservtion/{date}/{roomId}";
	}

	// 予約処理：github
//	@RequestMapping(method = RequestMethod.POST)
//	String reserve(@Validated ReservationForm form, BindingResult bindingResult,
//			@AuthenticationPrincipal ReservationUserDetails userDetails, // core.annotation.AuthenticationPrincipal;
//			@DateTimeFormat(iso = DateTimeFormat.ISO.DATE) @PathVariable("date") LocalDate date,
//			@PathVariable("roomId") Integer roomId, Model model) {
//		if (bindingResult.hasErrors()) {
//			return reserveForm(date, roomId, model);
...
//		// reservation.setUser(dummyUser()); // dummyUser でテスト
//		try {
//			reservationService.reserve(reservation);
//		} catch (UnavailableReservationException | AlreadyReservedException e) {
//			model.addAttribute("error", e.getMessage());
...
//	}
	...
	String cancel(@RequestParam("reservationId") Integer reservationId, @PathVariable("roomId") Integer roomId,
		...
		// 予約取り消し：Use dummyUser() for spring security
		User user = dummyUser();
		try {
