public class Board {
	...
	/** color色の石をを(row, column)に置いて，置けたら true を返す */
	...
	/** Compute the delta for the row and column. At all times, only the row or the column
	...
	/** Recursively flip the remainderof the row. If -1 is returned, then we know we hit the boundary 
	    of the row (or a null piece) before we found our own color, so there's nothing to flip. 
	    Return the error code.  */
	int flipped = flipSection(row + r, column + r, color, d);
	if (flipped < 0) {
	    return -1;
	}
	...
	/* If we added x pieces of a color, then we actually removed x - 1 pieces of the other color.
	   The -1 is because one fo the new pieces was the just-placed one. */
